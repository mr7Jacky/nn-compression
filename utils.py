...
def dataset_builder(dataset, train=True, normalize=None, preproc=None, download=False):
    ...
    kwargs = {'transform': preproc, 'download': download}
    if dataset == 'ImageNet':
        kwargs['split'] = 'train' if train else 'val'
    else:
        kwargs['train'] = train

    return _constructors[dataset]("./data", **kwargs)
